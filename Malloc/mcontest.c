...
int main(int argc, char **argv)
{
	...
	char *buffer = calloc(1, sizeof(alloc_stats));
	write(fd, buffer, sizeof(alloc_stats));
	close(fd);
	free(buffer);
	...
	alloc_stats *stats = mmap(NULL, sizeof(alloc_stats), PROT_READ, MAP_SHARED, fileno(file), 0);
	if (!stats)
	{
		perror("mmap");
		return 5;
	...
	munmap(stats, sizeof(alloc_stats));
	unlink(file_name);
	return 0;
}
